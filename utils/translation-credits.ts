...
import { getTranslatorIds, normalizeCredit, NormalizedCredits } from './helpers/translation.js';
import {
  ProjectData,
  ProjectListResponse,
  ProjectReportDataResponse,
  ProjectTranslationProgressResponse,
} from './helpers/crowdin-api-types.js';
import { crowdinApiRequest } from './helpers/crowdin-api-request.js';
...
void (async () => {
  ...
  rawReportData.data.forEach((reportDataItem) => {
    ...
    if (reportDataItem.languages.length > 0) {
      const userLanguages = reportDataItem.languages.reduce((acc: Locale[], language) => {
  ...
  const assembledReportDataOutputPath = path.join(readmeFolder, 'src', 'locales', 'crowdin.json');
  ...
  const sortedConfigs = (Object.entries(localeConfig.languages) as [Locale, LatestLanguageConfigType][]).sort(
